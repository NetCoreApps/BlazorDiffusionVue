...
const signInUrl = `/Account/Login?ReturnUrl=${encodeURIComponent(location.pathname)}`
        ...
        <div class="w-full">
            <div class="flex flex-col w-full border border-gray-700 rounded bg-gray-900 overflow-hidden">
                ...
                <div class="flex justify-between p-2 pl-4 bg-black items-center">
                    <div>
                        <a href="/community-rules" target="_blank" class="text-sm text-gray-400">read the community rules</a>
                    </div>
                    <div>
                        <span class="mr-2 text-sm text-gray-400">{{ remainingChars }}</span>
                        <PrimaryButton @click="submit" :disabled="loading">Post</PrimaryButton>
                    </div>
                </div>
            </div>
...
export default {
    components: { Thread, Comment, NewReport, InputComment },
    template: /*html*/`
    <div :class="['mt-24 mx-auto flex flex-col w-full max-w-3xl transition-opacity', AppData.init ? 'opacity-100' : 'opacity-0']">
        ...
        <div v-else class="flex justify-center w-full">
            <div class="flex justify-between w-full max-w-2xl border border-gray-700 rounded bg-gray-900 overflow-hidden">
                ...
                <div>
                    <SecondaryButton :href="signInUrl" class="m-1">Sign In</SecondaryButton>
                </div>
            </div>
        </div>
